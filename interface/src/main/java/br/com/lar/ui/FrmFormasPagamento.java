...
public class FrmFormasPagamento extends AbstractInternalFrame {
	...
	private CampoPesquisa<Banco> pesquisaBanco;
	...
	public FrmFormasPagamento(Long permissaoPrograma, Long codigoUsuario) {
		...
		pesquisaBanco = new CampoPesquisa<Banco>(bancoService, PesquisaEnum.PES_BANCOS.getCodigoPesquisa(),
				getCodigoUsuario()) {

			private static final long serialVersionUID = 1L;

			@Override
			public String formatarValorCampo(Banco objeto) {

				return String.format("%d - Ag: %s CC:%s", objeto.getIdBanco(), objeto.getNumeroAgencia(),
						objeto.getNumeroConta());
			}
		};
		...
		getContentPane().add(pesquisaBanco, "cell 0 8,growx");
		...
		panelActions = new PanelActions<FormasPagamento>(this, formasPagamentoService,
			...
			public void carregarObjeto(FormasPagamento objeto) {
				...
				pesquisaBanco.setValue(objeto.getBanco());
			...
			public boolean preencherObjeto(FormasPagamento objetoPesquisa) {
				...
				objetoPesquisa.setBanco(pesquisaBanco.getObjetoPesquisado());
